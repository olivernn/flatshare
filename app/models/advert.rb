class Advert < ActiveRecord::Base 
  ...
  def self.featurable
    (find(:all, :joins => "INNER JOIN images ON images.advert_id = adverts.id", :select => 'DISTINCT adverts.*', :order => 'adverts.created_at DESC', :limit => 6) | find(:all, :order => 'adverts.created_at DESC', :limit => 6)).slice(0,5)
  end
